...
def extract_words(text):
    ...
    def extract_char(char):
    	extracted=''
    	for letter in char:
    		if not(str(letter) in ascii_letters):
    			extracted=extracted+' '
    		else:
    			extracted=extracted+letter
    	return extracted
...
def get_word_sentiment(word):
    ...
    0.875
    >>> sentiment_value(get_word_sentimente('bad'))
    -0.625
...
def analyze_tweet_sentiment(tweet):
    ...
    for word in text:
    	...
    	if  has_sentiment(sentiment):
    		sentiments.append(sentiment_value(sentiment))
    if len(sentiments)==0:
    	return make_sentiment(None)

    return make_sentiment(average(sentiments))
...
def find_centroid(polygon):
    ...
    def area(polygon):
    	...
    	while i<len(polygon)-1:
    		pos1=polygon[i]
    		pos2=polygon[i+1]
    		total+=latitude(pos1)*longitude(pos2)-latitude(pos2)*longitude(pos1)
    		i+=1
    	return total/2
    ...
    while i<len(polygon)-1:
    	pos1=polygon[i]
    	pos2=polygon[i+1]
    	temp=(latitude(pos1)*longitude(pos2)-latitude(pos2)*longitude(pos1))

    	totalx+=(latitude(pos1)+latitude(pos2))*temp
    	totaly+=(longitude(pos1)+longitude(pos2))*temp
    	i+=1

    if area(polygon)==0:
    	return [latitude(polygon[0]),longitude(polygon[0]),0.0]

    return [totalx/(6*area(polygon)),totaly/(6*area(polygon)),abs(area(polygon))]
...
def find_state_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    def sum_centroid(coordinate): #coordinate=0 for latitude, coordinate=1 for longitude
    	total_sum=0
    	for polygon in polygons:
    		data=find_centroid(polygon)
    		total_sum+=data[coordinate]*data[2]
    	return total_sum
    ...
    return make_position(sum_centroid(0)/area_sum,sum_centroid(1)/area_sum)
    ...
    for tweet in tweets:
    	...
    	for state in us_states:
    		...
    		if min_distance==-1:
    			...
    			min_distance=distance
    			closest_state=state
    	lst.append([closest_state,tweet])
